...
def has_tracked_params(model):
    attn = model.block.attn
    return any(hasattr(attn, name) for name in ["tau_raw", "alpha", "beta_raw"])


def record_params(model):
    attn = model.block.attn
    snap = {}
    for name in ["tau_raw", "alpha", "beta_raw"]:
...
def run_training(model, train_x, train_y, config, quiet=False):
    ...
    for epoch in range(config["epochs"]):
        ...
        if (epoch + 1) % 10 == 0 and has_tracked_params(model):
